...
# 요소가 한 개일 때(a, ) - 콤마를 찍어야 함

import sqlite3

def getconn():
    # db가 없으면 생성되고 있으면 연결
    conn = sqlite3.connect('./output/book2db.db')
    return conn
...
def insert_book():
    ...
    cursor.execute(sql, ("점프 투 파이썬", "박응용", 12600))
    conn.commit()
    conn.close()
...
def update_book():
    conn = getconn()
    cursor = conn.cursor()
    # 천 개의 파랑의 가격을 15000원으로 변경하기
    sql = "UPDATE book SET price = ? WHERE title = ?"
    cursor.execute(sql,(15000, '천 개의 파랑'))
    conn.commit()
    conn.close()
